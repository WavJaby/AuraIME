...
impl AnimationState {
    ...
    pub fn update_on_status_change(&mut self) -> bool {
        let now = Instant::now();
        let mut skip_fade_in = false;
        if let Some(start) = self.start_time {
            let elapsed = start.elapsed();
            // If it's already in "Fully visible" phase, skip fade-in by starting from Phase 2
    ...
    pub fn get_alpha_and_offset(&self) -> (f32, f32) {
        ...
        if elapsed < FADE_IN_DURATION {
            ...
            let t = (elapsed - (FADE_IN_DURATION + SHOW_DURATION)).as_secs_f32()
